...
if($ok==1)
{
	...
	$sql="insert into utilizatori(nume,prenume,email,parola,activ,token) values('".$nume."','".$prenume."','".$email."','".$parola."',0,'".$token."')";
	if(mysqli_query($con,$sql))
	{
		$last_id = mysqli_insert_id($con);
		$id = base64_encode($last_id);
		
		$cale = "http://localhost:8080/infoed/";
//		$cale = "https://infoed.rememberall.ro/";
		
		$mesaj = "Bine ati venit pe platforma InfoEd!<br><br>
						Pentru a va activa contul va rugam sa apasati pe link-ul urmator de mai jos:<br><br>
						<a href=".$cale."verifica.php?id=$id&cod=$token>Click AICI pentru activarea contului!</a>
						<br><br>
						Va multumim,";
						
			$subiect = "Activare cont pe InfoEd";
			trimite_mail($email,$mesaj,$subiect);
			header("location: mesaj_creare_cont.php");
	}
	else
	{
		echo "Nu s-a creat inregistrarea in tabela de utilizatori";
	}
}
