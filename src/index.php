...
if(isset($_POST['cmd_login'])) 
{
    ...
    if($responseKeys["success"])
	{
      ...
      // Verificam daca adresa de mail exista in baza de date
	  $sql = "SELECT COUNT(*) as cate FROM utilizatori WHERE email = '$email'";
	  ...
	  if($cate > 0)
	  {
		...
		if($id > 0)
		{
	      	$sql = "SELECT activ FROM utilizatori WHERE id_utilizator = ".$id;
	      	$result = mysqli_query($con,$sql);
	      	$row = mysqli_fetch_assoc($result);   
			$activ = $row['activ'];
			if($activ==1)
			...
			{
				$eroare = "<div class='LoginForm' style='font-size:11px; color:#cc0000; margin-top:10px;'>Contul nu este activat! Verificati-va mail-ul pentru link-ul de activare.</div>";
	  ...
	  {
		$eroare = "<div class='LoginForm' style='font-size:11px; color:#cc0000; margin-top:10px;'>Adresa de email nu exista in baza de date!</div>";
        $script = "<script> $(document).ready(function(){ $('#LoginForm').modal('show'); }); </script>";	  	
	  }
